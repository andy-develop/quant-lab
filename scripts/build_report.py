...
<p><b>股票池过滤</b>：按<b>信号日当天的 ST 状态</b>剔除 ST、*ST 个股（baostock 逐日 isST 历史，2026-09-08 修复了原先用"当前名称"过滤全历史的未来函数——戴帽前的健康期被误删、摘帽股被误留；修复后 A/B：开模式 +32.2%→+50.9%，回撤 -47.7%→-45.9%）；当前名称含"退"且无退市日期记录的个股整段剔除（保守兜底）；<b>已知退市日期 (out_date) 的个股仅剔除退市前 30 天起的区间</b>，此前健康期信号保留（2026-09-09 修复原"整段剔除"的轻微未来函数）；北交所因数据源不支持天然不含。全池基于 baostock 日线（含退市股，规避幸存者偏差）。价格口径：信号与收益使用<b>后复权序列</b>（历史值永久冻结，任意日期重跑结果可复现），成交明细展示不复权真实价；流动性过滤使用<b>真实成交额</b>（非前复权近似）。</p>
