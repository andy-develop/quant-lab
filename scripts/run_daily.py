...
if __name__ == "__main__":
    ...
    step("1/4 每日K线快照增量", daily_update.update_kline, fatal=True)
    step("2/4 涨停池/炸板池", daily_update.update_pool)   # 池子缺失不影响信号, 可降级
    ...
    step("3/4 信号扫描 + 组合回测(双口径)", scan_and_backtest, fatal=True)
    step("4/4 生成报告", build_report.main, fatal=True)   # 报告失败时中止, 避免上传过期报告
